...
- member_summary는 **(1) 회원 상태를 짚고 (2) 그래서 이번 시퀀스를 왜 이렇게 구성했는지(무엇을 고려했는지)**를 합쳐 **2줄 정도로 짧게** 쓴다. 회원 분석에서 끝내지 말고 처방 의도를 담되, 강사가 편하게 읽도록 한국어로 풀어서 — 영어 동작·근육명 나열이나 대시(-)·괄호 남발 없이 매끄럽게. 상세 설명보다 blocks(실제 시퀀스)가 본체다.

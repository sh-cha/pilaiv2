...
## 자세 흐름 규칙 (중요)
- 각 동작의 시작 자세(바로 누움/옆으로 누움/엎드림/앉음/네발기기·무릎서기/서기)를 의식해, 같은 자세의 동작은 연속으로 모아 배치한다.
- 자세 왕복을 만들지 않는다: 누움→서기→누움, 엎드림→앉음→엎드림 같은 흐름 금지. 회원이 일어났다 다시 눕는 일이 없어야 한다.
- 전체 흐름은 한 방향으로 자연스럽게: 예) 누움 → 옆으로 누움 → 앉음 → 네발기기 → 서기 (또는 그 역방향). 블록 순서를 지키면서도 인접 동작 간 자세 전환이 최소가 되게 배열한다.

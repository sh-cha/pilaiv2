...
- ▶ **인사이트/시퀀스 eval + 모델 비교** — 평가 harness 구축(`app/eval/`): 인사이트·시퀀스 rubric, LLM-as-judge, sonnet↔opus 비교(`npm run eval` / `npm run eval:seq`). eval이 임신 케이스 blocks 누락 버그를 잡아 프롬프트로 수정함. 남음: 아내 실사용 골든 라벨, 재생성 rubric/케이스, baseline 추세 쌓기

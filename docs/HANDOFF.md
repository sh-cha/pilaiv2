...
- 🔜 **앱 아이덴티티·견고성 [#14]** — Pilai 아이콘/스플래시(세이지), SafeArea(현재 paddingTop:54 하드코딩), 오프라인·에러 처리
...
- 보류: 금기/부하태그 verifier(Phase 1.5). 정리거리: 카탈로그 탭 제거로 `CatalogScreen` 함수는 미사용(코드 남아있음).

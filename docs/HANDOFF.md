...
- ▶ **인사이트/시퀀스/재생성 eval + 모델 비교** — harness 3종(`npm run eval`/`eval:seq`/`eval:regen`). eval이 **임신 blocks 누락**(수정함)·**재생성 과교정**(원본 미주입, 미수정)을 발견. 남음: 재생성 과교정 fix(원본 주입), 아내 실사용 골든 라벨, baseline 추세 쌓기
